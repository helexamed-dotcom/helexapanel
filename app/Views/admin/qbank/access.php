...
<div class="qb-page">
    <section class="qb-hero">
        <div class="qb-hero-main">
            <div class="qb-hero-icon"><?php \HeleXa\Core\View::partial('partials.icon', ['name' => 'key']); ?></div>
            <div>
                <h2>دسترسی دانشجویان به بانک سوال</h2>
                <p>پیش‌فرض هر دانشجو <strong>بدون دسترسی</strong> است. برای هر نفر مشخص کن کدام درس‌ها برایش باز باشد.</p>
            </div>
        </div>
    </section>

    <section class="qb-section">
        <form method="get" action="/admin/qbank/access" class="qb-filters" style="margin-bottom:14px;">
                ...
                ?>
                    <div class="qb-row">
                        ...
                        <div class="qb-row-actions">
                            <a class="btn btn-primary btn-sm" href="/admin/qbank/access/<?= e($student['uuid']) ?>">تنظیم دسترسی</a>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
                ...
                <nav class="pager" aria-label="صفحه‌بندی">
                    <?php for ($p = max(1, $page - 3); $p <= min($pages, $page + 3); $p++): ?>
                        <a class="pager-item<?= $p === $page ? ' is-current' : '' ?>"
                           href="/admin/qbank/access?<?= e(http_build_query(array_filter(['q' => $search]) + ['page' => $p])) ?>"><?= e(fa((string) $p)) ?></a>
                    <?php endfor; ?>
